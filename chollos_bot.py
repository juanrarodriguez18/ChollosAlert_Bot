...
@click.option('--refresh_chollos', help='The time between chollos checks')
@click.option('--log_level', help='Level to log. [INFO, DEBUG]')
@click.option('--log_path', help='The bot token. Please talk with @BotFather')
def init(config_path, token, admin_user_id, admin_username, db_path, refresh_chollos, log_level, log_path):

    set_config(Config())
    if config_path:
        get_config().load_config_file(config_path)
    get_config().load_config_variables(token, admin_user_id, admin_username, db_path, refresh_chollos, log_level, log_path)
